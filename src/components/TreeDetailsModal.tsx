...
interface TreeDetailsModalProps {
  ...
  tree: Tree | null;
  isOwner: boolean;
  onEdit: (id: string, nickname: string, description: string, scientificTreeId: string | null) => Promise<void>;
...
export default function TreeDetailsModal({
  ...
  useEffect(() => {
    if (tree) {
      setNickname(tree.nickname || '');
      setDescription(tree.description || '');
      setScientificTreeId(tree.scientific_tree_id || null);
      setIsEditing(false);
      setError('');
    }
  }, [tree]);
  ...
  useEffect(() => {
    if (!isOpen || !mapRef?.current || !tree) return;
    ...
    const updatePosition = () => {
      try {
        ...
        const point = map.project([tree.longitude, tree.latitude]);
  ...
  const handleSave = async () => {
    ...
    try {
      await onEdit(tree.id, nickname.trim(), description.trim(), scientificTreeId);
      setIsEditing(false);
    } catch (err: any) {
  ...
  const handleDelete = async () => {
    if (!confirm('Are you sure you want to delete this tree marker?')) {
      return;
    }
      ...
      >
        ...
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-bold text-gray-800">
            {isEditing ? 'Edit Tree Marker' : 'Tree Details'}
          </h2>
          <button
            onClick={onClose}
          ...
          {isEditing ? (
            ...
            <>
              <div>
                ...
                <p className="text-gray-900 text-lg font-semibold">{tree.nickname}</p>
              </div>

              {tree.scientific_tree && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Scientific Name
                  </label>
                  <p className="text-gray-900">
                    <span className="italic">{tree.scientific_tree.scientific_name}</span>
                    {tree.scientific_tree.common_name && (
              ...
              {tree.description && (
                <div>
                  ...
                  <p className="text-gray-700 whitespace-pre-wrap">{tree.description}</p>
                </div>
              )}

              <div className="text-xs text-gray-500">
                Location: {tree.latitude.toFixed(6)}, {tree.longitude.toFixed(6)}
              </div>
